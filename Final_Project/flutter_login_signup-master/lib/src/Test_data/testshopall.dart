...
class ShopShow02 extends StatefulWidget {
  @override
  _ShopShow02State createState() => _ShopShow02State();
}

class _ShopShow02State extends State<ShopShow02> {
    ...
    await Dio().get(url).then((value) {
      ...
      if (value.toString() != 'null') {
        ...
        for (var map in result) {
          ...
          setState(() {
            String url2 =
                '${MyUrlpath().urlpath}/Final_Project/flutter_login_signup-master/lib/src/Connection_DB/faveritshop/userandshop.php?isAdd=true&ID_User=$usertouser&ID_Shop=${datashop.shopId}';
            Dio().get(url2).then((value) {
              if (value.toString() != 'null') {
                check = true;
                // var result2 = json.decode(value.data);
                shopCard.add(showpageshop(datashop, index, check));
                index++;
              ...
              child: GestureDetector(
                onTap: () {
                  Navigator.pushAndRemoveUntil(
                      context,
                      MaterialPageRoute(builder: (context) => ShopShow02()),
                      (route) => false);
                },
  ...
  Widget showpageshop(ShopModel datashop, int index, bool check) {
