...
import 'package:flutter_login_signup/src/UserAdmin/unbanshop.dart';
...
class _BanPageState extends State<BanPage> {
  ...
  Future<Null> banshopCactus(ShopModel getban) async {
    showDialog(
      context: context,
      builder: (context) => SimpleDialog(
        title: MyStyle()
            .showTitleH2('คุณต้องการระงับร้านค้า :  ${getban.nameShop} ??'),
        children: <Widget>[
          Row(
            ...
            children: <Widget>[
              FlatButton(
                onPressed: () async {
                  ...
                  String token;
                  String url =
                      '${MyUrlpath().urlpath}/Final_Project/flutter_login_signup-master/lib/src/Connection_DB/adminsetting/banshop.php?isAdd=true&Shop_id=${getban.shopId}&Status=Banshop';
                  await Dio().get(url).then((value) => ShowshopAll());

                  String title = 'ร้านค้า : ${getban.nameShop} ถูกระงับแล้ว ';
                  String body =
                      'ร้านค้าของท่านถูกระงับเนื่องจากพฤติกรรมที่ไม่เหมาะสมบางประการ';
            ...
            Padding(
                ...
                child: GestureDetector(
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => UnBanPage()),
                    );
                  },
                  child: Icon(
